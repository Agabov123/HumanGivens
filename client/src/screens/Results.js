...
export function FoodSafety({ navigation }) {
  ...
  const search = () => {
    setLoading(true);
    fetch(`http://localhost:8000/RatfoodSafety?text=${text}`)
  ...
  const guineaSearch = () => {
    setLoading(true);
    fetch(`http://localhost:8000/GuineafoodSafety?text=${text}`)
      .then((response) => response.json())
      .then((data) => {
        setData(data);
        setLoading(false);
      ...
      return (
        <SafeAreaView style={styles.SafeArea}>
          <ScrollView>
            <Image
              style={WelcomeBannerStyle.image}
              source={require("../../assets/guineaPig.png")}
            />
            <View style={styles.container}>
              <View style={ButtonStyles.button}>
                <Text style={ButtonStyles.buttonTextLarge}>
                  Guinea Pig Food Checker:
                </Text>
              </View>
              <TextInput
                style={styles.textBox}
